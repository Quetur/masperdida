...
router.get("/modificarmascota/:id", async (req, res) => {
    try {
        ...
        const [proRows] = await pool.query(
            "SELECT *,latitud AS lat, longitud AS lng FROM mascota WHERE id_mascota = ?",
            [id]
        );
        const pro = proRows[0];

        // 2. Categorías (Perdido, Encontrado, etc.)
        const [cat] = await pool.query(
          `SELECT c.id_categoria, c.des, 
           IF(p.id_categoria = c.id_categoria, 'S', '') AS id_categoria_mascota 
           FROM categoria c 
           CROSS JOIN (SELECT id_categoria FROM mascota WHERE id_mascota = ?) p`,
          [id]
        );

        // 3. Tipos (Tu consulta: Perro, Gato, etc.)
        const [tipo] = await pool.query(
          `SELECT t.id_tipo, t.des, 
           IF(p.id_tipo = t.id_tipo, 'S', '') AS id_tipo_mascota 
           FROM tipo t 
           CROSS JOIN (SELECT id_tipo FROM mascota WHERE id_mascota = ?) p`,
          [id]
        );

        // 4. Razas (Para el filtrado inicial en el JS)
        const [subcat] = await pool.query(
          `SELECT r.id_raza, r.id_tipo, r.des, 
           IF(p.id_raza = r.id_raza, 'S', '') AS id_raza_mascota 
           FROM raza r 
           CROSS JOIN (SELECT id_raza FROM mascota WHERE id_mascota = ?) p`,
          [id]
        );

        res.render("modificarmascota", { 
            pro, 
            cat, 
            tipo, // <--- No te olvides de pasar 'tipo' al render
            subcat, 
